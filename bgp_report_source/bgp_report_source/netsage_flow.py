...
def get_sensor_flow_entries(start, end, es_instance):
	...
	number_of_sensors = es_object.search(index="om-ns-netsage*", body=query, request_timeout=30)["aggregations"]["count_sensor_id"]["value"]
	print "SENSORS:\t{}".format(number_of_sensors)
	...
	return es_object.search(index="om-ns-netsage*", body=query, request_timeout=30)["aggregations"]["group_by_sensor_id"]["buckets"]
...
def get_events_flow_entries(start, end, es_instance):
        ...
        es_object = Elasticsearch([es_instance])
        query = {"query":{"range": {"start": {"gte":start, "lte":end, "format":"epoch_millis"}}},\
                "size": 0, "aggs":{"group_by_src_ip":{"terms":{"field":"meta.src_ip.keyword"}, "aggs":{"total_bits":\
                {"sum":{"field":"values.num_bits"}}}}}}

        return es_object.search(index="om-ns-netsage*", body=query, request_timeout=30)["aggregations"]["group_by_src_ip"]["buckets"]
